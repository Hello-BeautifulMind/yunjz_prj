$(function() {
	...
	$("#del-btn").click(function() {
		$("#myModal").modal("hide");
		var del_list = $("input:checkbox[name='del_id']:checked").map(function() { return this.value}).get();
		var csrf_token = $("input[name=csrfmiddlewaretoken]").val();
        $.ajax({
			url: ITEMS_URL,
			dataType: "json",
			method: "POST",
			data: {"csrfmiddlewaretoken":csrf_token, "del_id": del_list},
			success: function(ret) {
				$.each(ret, function(i, id) {
					$("#item"+id).hide();	// 隐藏行
				})
			}
		})
	...
	$("#search-btn").click(function() {
		...
		if("0" == category_search) {
			...
			$.ajax({
				...
				beforeSend: function() {
					$("#search-btn").addClass("disabled");	// 发送ajax后暂时禁用按钮
...
function update_node(ret) {
	...
	if( ret.length >0 ) {	// 
		...
		$.each(ret, function(i, item) {
			var new_node = $('<tr>' +
			'<td data-toggle="tooltip" data-placement="top" title="编辑该账单"><a href="' + EDIT_ITEM_URL + '"></a></td>' + 
			'<td data-toggle="tooltip" data-placement="top" title="编辑该账单"><a href="#"></a></td>' +
			'<td data-toggle="tooltip" data-placement="left" title="显示该分类下的账单"><a href="' + EDIT_CATEGORY_URL + '"></a></td>' +
			'<td></td>' +
			'<td><input type="checkbox" name="del_id" value="" autocomplete="off"/></td>' +
			'</tr>');
			new_node.attr("id", "item"+item["pk"]);
			...
			a3.attr("href", a3.attr("href").replace("0", item["fields"]["category"][0]));
			a1.text(item["fields"]["pub_date"]);
			a2.text(item["fields"]["price"]);
			a3.text(item["fields"]["category"][1]);
			new_node.children("td").eq(3).text(item["fields"]["comment"]);
			new_node.find("input:first").val(item["pk"]);	
		...
		// POST请求才生成分页
		if(this["method"] == "POST") {
			var c_id = ('fields' in ret[0]) ? ret[0]['fields']['category'][0] : "";
			...
			pagination.append('<li class="page-turn"><span style="border:none; margin-left:20px; color:#999;">共<em id="pages-num">' + pages_num + '</em>页，到第</span><input type="text" id="skip-page" style="position:relative; float:left; margin-top: 1px; width: 40px; height:30px; border: 1px solid #ddd;" value="1" /><span style="border:none; padding-left:5px; color:#999;">页</span><a href="?c_id=' + ret[0]["fields"]["category"][0] + '&page=" id="gogo" onclick="skip_page(this.href); return false;" onfocus="this.blur();">跳转</a><span id="input-error" style="visibility: hidden; color: red; border:none;">请输入一个大于0的数字</span></li>');
		}
		// GET请求更新分页栏
		else {
